...
Separately, the upper-bound half of the forward direction,
\[
H^\circ \text{ a forest} \;\Longrightarrow\; \ex(n;H)=O(n),
\]
is now also formalized (module \texttt{Forest.lean}), via a greedy
leaf-by-leaf embedding of a $k$-vertex forest into any host of minimum
degree at least $k$; it supplies the Case~(ii) upper bound of the
structural variant described below. The remaining unformalized piece of
the classical biconditional is the converse (cycle case), whose known
proofs need probabilistic tools not found in the surveyed
\texttt{mathlib4} API.
% UPGRADE (after single-graph Omega(n) milestone): add that the full
% forward direction (forest with >= 2 edges => Theta(n)) is formalized,
% i.e. the Omega(n) half via star/matching host constructions.
...
\texttt{SharpConstant.lean}): any host graph containing neither
